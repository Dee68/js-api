const API_KEY = 'liT0bM6qBKA87nyE8ZW1-44XrWA';
const API_URL = 'https://ci-jshint.herokuapp.com/api';
const resultsModal = new bootstrap.Modal(document.getElementById('resultsModal'));
document.getElementById('status').addEventListener('click', e => getStatus(e));
document.getElementById('submit').addEventListener('click', e => postForm(e));
//Make a get request with the api key & pass the data to a display function
...
function displayStatus(data){
    ...
    document.getElementById('results-content').innerHTML = results + `<div class='key-status'>${data.expiry}</div>`;
    resultsModal.show();
}
// converts the options to comma seperated values
...
function displayException(data){
    ...
    results =`<div>The API returned status_code:${data.status_code}</div>`;
